...
export const uniformScale = (options: IUniformScale = {}) => {
  const {
    maxPos = { x: 0, y: 0, w: 100, h: 100 },
    transform = mat3.fromTranslation(mat3.create(), [0, 0]),
    scaleCenter = "CC",
    ratio = 1,
  } = options;
  // 1. 获取最大包围盒的位置信息
  ...
  switch (scaleCenter) {
    case "LT":
      vec2.copy(apivot, ap0);
      break;
    case "RT":
      vec2.copy(apivot, ap1);
      break;
    case "RB":
      vec2.copy(apivot, ap2);
      break;
    case "LB":
      vec2.copy(apivot, ap3);
      break;
    // 计算中点
    case "TC":
